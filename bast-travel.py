...
"""
	Description: 
		...
		that he doesn't want to drive more than t = 174 miles and 
		he will visit only 3 towns.

		Which distances, hence which towns, 
		they will choose so that the sum of the distances is 
		the biggest possible - to please Mary - but less than t - to please John- ?
	...
	Example:
		...
		k (number of towns to visit, k >= 1) and ls 
		(list of distances, all distances are positive or null integers and 
		...
		equal to the given limit t, if that sum exists, 
...
def choose_best_sum(t, k, ls):
	...
	for i in range(len(comb)):
		if t >= sum(comb[i]) and sum(comb[i]) > result_sum:
			result_sum = sum(comb[i])
			result = comb[i]
