...
"""
	Description:
		...
		Between 2 and 50 we have the following pairs of 2-gaps 
		...
		spaced with a gap of g between the limits m, n if 
		these numbers exist otherwise nil or null or None or Nothing 
...
def gap(g, m , n):
	p = []
	while  m <= n:
		if 0 not in [m % i for i in range(2, int(m**0.5) + 1)]:
			p.append(m)
			if len(p) > 1:
				if p[-1] - p[-2] == g:
					return [p[-2], p[-1]]
